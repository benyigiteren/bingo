# Biolink: Self-Hosted Minimalist Biyografi Platformu
...
./biolink -port 8080 -db ./data/biyografi.db
```

### 2. Ortam Değişkenleri
*   `PORT`: Sunucunun dinleyeceği port (örn: `8080`)
*   `DB_PATH`: SQLite veritabanı dosyasının tam yolu (örn: `/var/data/biolink.db`)
...
services:
  biolink:
    image: biolink:latest
    build: .
    container_name: biolink_app
    ports:
      - "1907:1907"                     # Dış Port:İç Port (Varsayılan 1907)
    environment:
      - PORT=1907
      - DB_PATH=/app/data/biolink.db
    volumes:
      - ./biolink_data:/app/data       # Kalıcı SQLite Veritabanı
      - ./biolink_uploads:/app/uploads # Kalıcı Profil ve Arka Plan Resimleri
...
go build -o biolink main.go

# Derleme (Windows için)
go build -o biolink.exe main.go
